...
from src.utils.spreadsheet import update_focus_targets, SPREADSHEET_ID, FOCUS_SHEET_NAME, FOCUS_HEADERS

# ── スプレッドシートから注目銘柄を読み込む ────────────────────────────────────
def load_focus_targets_from_sheet() -> list[dict]:
    """注目銘柄シートの現在の内容を読み込んで返す。"""
    try:
        from src.utils.spreadsheet import get_service
        service = get_service()
...
sheets_ready   = bool(spreadsheet_id and sa_json)
    ...
    with col_load:
        if st.button("📥 スプレッドシートから読み込む", use_container_width=True):
            with st.spinner("読み込み中…"):
                loaded = load_focus_targets_from_sheet()
            if loaded:
                st.session_state.focus_targets = loaded
                st.success(f"✅ {len(loaded)} 銘柄を読み込みました。")
                st.rerun()
            else:
                st.info("注目銘柄シートにデータがありませんでした。")
...
if not st.session_state.focus_targets:
    st.info("銘柄がまだ追加されていません。上の「読み込む」ボタンまたはフォームから追加してください。")
else:
...
if not sheets_ready:
    ...
    st.info("「スプレッドシートに反映」を押すと、注目銘柄シートの内容が**上のリストで上書き**されます。")
